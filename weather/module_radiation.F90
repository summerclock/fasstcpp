...
! local variables
      real(kind=8):: l,rv,eso,ea,ta,ematmp
...
! calculate the vapor pressure
      ...
      if(dabs(l/rv*(1d0/Tref - 1d0/ta)) < 5d1)                          &
        ea = (eso*dexp(l/rv*(1d0/Tref - 1d0/ta)))*rh*1d-2                !vapor pressure

! Calculate the emissivity

      if(ea > 0d0) then
        ematm = 1.24d0*(ea/ta)**(1d0/7d0)                                !special test of brutsaert
      else
        ematmp = 7d-1 + 5.95d-5*ea*exp(1.5d3/ta)                         !idso 
        ematm = -7.92d-1 + 3.161d0*ematmp-1.573*ematmp*ematmp            !idso with watchmann corr
      end if
...
! rg: Albedo of ground (rg=albedo_fasst).
...
!  eliptical orbit. Redone daily at 0500 hours.
      sdowne = 2d0*pi*real(jday - 2)/365.242d0
